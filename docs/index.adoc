...
SIGN::
    specifies a digitive or positive number, 0 being positive and 1 being
    negative.

EXPONENT::
    Takes the unsigned version of the number and counts the first bit to the
...
FRACTION::
    Takes the unsigned version number and stores the first bit to the right of
    most signicant on bit all the way to the least significant bit.
