...
downward direction. Items are placed on the stack with the https://www.felixcloutier.com/x86/push[push] instruction
and removed from the stack with the https://www.felixcloutier.com/x86/pop[pop] instruction.

=== Virtual Memory

From https://linux.die.net/man/5/elf[man elf] we can see that `e_entry` from the `ElfN_Ehdr` struct gives the
