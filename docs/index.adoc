...
Must start wih a function prologue and end with a function epilogue followed by
a `pop rip`.
...
Must move to the the function label with a jmp, like so:
...
The first six arguments must be rdi, rsi, rdx, rcx, r8, and r9, respectively.

One should not invoke `ret` (`pop rip`) unless the stack is in exactly the same
state as when the function started.

Must restore all other register that where not arguments.
...
`ret`::
    Denotes the function end. same as `pop rip`.

`leave`::
    Is the the same as `mov rsp, rbp` and `pop rbp` (the epilogue).

`call <address>`::
    Is the same as `push rip` and `jmp <address>`

`enter 0, 0`::
    Is the same as `push rbp` and `mov rbp, rsp` (the prologue). It
    is said to have poor performance.
...
Leaf Functions::
    Execute some commands and then return to the parent function.

=== Alignment
The stack has to have 16-byte alignment when you call a function. Inside a leaf
function, you can in general ignore stack from and stack alignment; it is not
necessary as long as you don't mess with the stack. Similarly, if your function
does not use SIMD instructions, you do not need to care about alignment.
Compilers have optimizing functionality, and sometimes when you look at code
generated by compilers, you will find that there was no stack from used. That
happens when the compilers noticed during optimization that a stack form is no
needed. It **IS** a good habit to always include a stack from and check the
stack alignment; it can save you a lot of trouble later. A good reaason to
include a stack frome is the fact that GDB and GDB-based debugges (such as DDD
and SASM) expect to find a stack frame. If there is no stacfk form in your code,
the debugger will behave unpredictably, such as ignoring breakpoints or jumping
over instructions.
...
checkout this https://hackeradam.com/x86-64-linux-syscalls/[syscall table] for
linux
...
== NASM and GDB
=== Get Memory Allocated On The Heap

List memory regions mapped by the specified process
- https://www.baeldung.com/linux/find-heap-memory-running-process[find-heap-memory-running-process]
...
====
    ...
    Mapped address spaces:
          ...
          0x555555554000     0x555555555000     0x1000        0x0  r--p   /usr/local/share/tst/c/tst
          0x555555555000     0x555555556000     0x1000     0x1000  r-xp   /usr/local/share/tst/c/tst
          0x555555556000     0x555555557000     0x1000     0x2000  r--p   /usr/local/share/tst/c/tst
          0x555555557000     0x555555558000     0x1000     0x2000  r--p   /usr/local/share/tst/c/tst
          0x555555558000     0x555555559000     0x1000     0x3000  rw-p   /usr/local/share/tst/c/tst
          0x555555559000     0x55555557a000    0x21000        0x0  rw-p   [heap]
          0x7fffb7d74000     0x7ffff7d78000 0x40004000        0x0  rw-p   
          0x7ffff7d78000     0x7ffff7da0000    0x28000        0x0  r--p   /usr/lib/x86_64-linux-gnu/libc.so.6
          0x7ffff7da0000     0x7ffff7f35000   0x195000    0x28000  r-xp   /usr/lib/x86_64-linux-gnu/libc.so.6
          0x7ffff7f35000     0x7ffff7f8d000    0x58000   0x1bd000  r--p   /usr/lib/x86_64-linux-gnu/libc.so.6
          0x7ffff7f8d000     0x7ffff7f8e000     0x1000   0x215000  ---p   /usr/lib/x86_64-linux-gnu/libc.so.6
          0x7ffff7f8e000     0x7ffff7f92000     0x4000   0x215000  r--p   /usr/lib/x86_64-linux-gnu/libc.so.6
          0x7ffff7f92000     0x7ffff7f94000     0x2000   0x219000  rw-p   /usr/lib/x86_64-linux-gnu/libc.so.6
          0x7ffff7f94000     0x7ffff7fa1000     0xd000        0x0  rw-p   
          0x7ffff7fbb000     0x7ffff7fbd000     0x2000        0x0  rw-p   
          0x7ffff7fbd000     0x7ffff7fc1000     0x4000        0x0  r--p   [vvar]
          0x7ffff7fc1000     0x7ffff7fc3000     0x2000        0x0  r-xp   [vdso]
          0x7ffff7fc3000     0x7ffff7fc5000     0x2000        0x0  r--p   /usr/lib/x86_64-linux-gnu/ld-linux-x86-64.s
    o.2
          0x7ffff7fc5000     0x7ffff7fef000    0x2a000     0x2000  r-xp   /usr/lib/x86_64-linux-gnu/ld-linux-x86-64.s
    --Type <RET> for more, q to quit, c to continue without paging--
    o.2
          0x7ffff7fef000     0x7ffff7ffa000     0xb000    0x2c000  r--p   /usr/lib/x86_64-linux-gnu/ld-linux-x86-64.s
    o.2
          0x7ffff7ffb000     0x7ffff7ffd000     0x2000    0x37000  r--p   /usr/lib/x86_64-linux-gnu/ld-linux-x86-64.s
    o.2
          0x7ffff7ffd000     0x7ffff7fff000     0x2000    0x39000  rw-p   /usr/lib/x86_64-linux-gnu/ld-linux-x86-64.s
    o.2
...
From the heap memory addresses we can use `find` to find a string within the heap

[source, gdb]
----
find 0x555555559000, 0x55555557a000, 'h', 'e', 'l', 'l', 'o', '1'
----
